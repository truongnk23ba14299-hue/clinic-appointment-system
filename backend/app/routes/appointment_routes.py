...
def create_appointment():
    """Bệnh nhân đặt lịch khám (hỗ trợ tự điền ngày giờ)"""
...
def get_appointments():
    ...
    - ADMIN: Xem tất cả (có lọc theo doctor_id, patient_id, status, date)
    """
...
@token_required
def update_appointment_full(appointment_id):
    """
    Cập nhật toàn bộ (Full update) lịch hẹn:
    - Bệnh nhân cập nhật ngày, giờ, lý do
    - Bác sĩ cập nhật trạng thái (PENDING -> CONFIRMED -> COMPLETED), ngày, giờ, lý do
    - Admin toàn quyền cập nhật
    """
    data = request.get_json() or {}
    res, status_code = AppointmentService.update_appointment_full(request.current_user, appointment_id, data)
    return jsonify(res), status_code

@appointment_bp.route('/<int:appointment_id>/status', methods=['PUT'])
@token_required
def update_appointment_status(appointment_id):
    """Tương thích ngược: đổi status"""
    data = request.get_json() or {}
    new_status = data.get('status')
    if not new_status:
        return jsonify({'success': False, 'message': 'Thiếu tham số status mới'}), 400

    res, status_code = AppointmentService.update_appointment_full(request.current_user, appointment_id, {'status': new_status})
    return jsonify(res), status_code

@appointment_bp.route('/<int:appointment_id>', methods=['DELETE'])
@token_required
def cancel_appointment(appointment_id):
    """Bệnh nhân, Bác sĩ hoặc Admin hủy lịch hẹn hợp lệ"""
